...
class TestLoadCheckpoint:
    ...
    def test_raises_error_when_no_arguments_provided(self):
        with pytest.raises(
            ValueError, match="Must provide either 'model_name' or 'ckpt_path'"
        ):
            load_checkpoint()

    def test_raises_error_for_unknown_model_name(self):
        with pytest.raises(ValueError, match="Unknown model"):
            load_checkpoint(model_name="nonexistent_model")

    def test_raises_error_for_nonexistent_checkpoint_file(self):
